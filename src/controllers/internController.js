...
const createIntern = async function (req, res) {
    try {
        ...
        if (getEmail) { return res.status(400).send({ status: false, msg: "Email is already present.Enter a new email" }) }
        if (!mobile) { return res.status(400).send({ status: false, msg: "mobile no. is mandatory" }) }
        if (!(/^\d{10}$/.test(mobile))) { return res.status(400).send({ status: false, msg: "Enter valid mobile no." }) }

        let getMobile = await internModel.findOne({ mobile })
        if (getMobile) { return res.status(400).send({ status: false, msg: "Mobile no. is already present.Enter a new moile no." }) }
